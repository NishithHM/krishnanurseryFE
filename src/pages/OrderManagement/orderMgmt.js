...
import { addLink, addTitle, formatOrdersData, ROLE_TABLE_HEADER } from "./helper";
...
const OrderMgmt = () => {
    ...
    const loadInitialOrders = async () => {
        ...
        const formattedData = formatOrdersData({ data: list.data, role: user.role, onAction })
    ...
    const searchHandler = debounce(async (query) => {
        ...
        if (query.length >= 3) {
            ...
            const counts = await getOrders({ body: { search: query, isCount: true } })
            setOrdersCount(get(counts, "data[0].count", 0))
            const list = formatOrdersData({ data: res.data, role: user.role, onAction });
            setData(list);
        ...
        <div>
            ...
            <div className={styles.wrapper}>
                ...
                <div className={styles.paginationContainer}>
                    {['procurement', 'sales'].includes(user.role) && <Link to={addLink[user.role]}>
                        <div>
                    ...
                    <div className={styles.paginationInner}>
                        {/* count */}
                        <span>{`${page === 1 ? "1" : (page - 1) * 10}-${page * 10 > ordersCount ? ordersCount : page * 10
                            } of ${ordersCount}`}</span>
                        {/* controls */}
                        <button
                        ...
                        <button
                            onClick={() => setPage((e) => e + 1)}
                            disabled={
                                (page * 10 > ordersCount ? ordersCount : page * 10) >= ordersCount
                            }
                            className={styles.paginationControls}
                        >
